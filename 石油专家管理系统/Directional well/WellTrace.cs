...
namespace OilPressCtrlES.Calcuation
{
    ...
    public class WellTrace
    {
        ...
        public void readWellTraceDatafromDB(string WellNo)
        {
            ...
            this.strWellNo = WellNo;
            DataTable dtWellNo = DataOpp.ExecuteDataTable(CommonSql.strSelectWellTraceCount + "'" + WellNo + "'");
            NT = int.Parse(dtWellNo.Rows[0]["WellTraceCount"].ToString()); //行数
            SHEN = new double[NT + 1];//计算从下标1开始 
            ALFA = new double[NT + 1];
            FAI = new double[NT + 1];
            DataTable dtWellData = DataOpp.ExecuteDataTable(CommonSql.strSelectWellTrace + "'" + WellNo + "'");
            //遍历datatable 
